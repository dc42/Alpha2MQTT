...
modbusRequestAndResponseStatusValues RegisterHandler::readHandledRegister(uint16_t registerAddress, modbusRequestAndResponse* rs)
{
	...
	default:
	{
		...
		rs->statusMqttMessage = MODBUS_REQUEST_AND_RESPONSE_NOT_HANDLED_REGISTER_MQTT_DESC;
		rs->displayMessage = MODBUS_REQUEST_AND_RESPONSE_NOT_HANDLED_REGISTER_DISPLAY_DESC;
		break;
	}
	}
											...
											if (result == modbusRequestAndResponseStatusValues::readDataRegisterSuccess)
											{
												...
												if (result == modbusRequestAndResponseStatusValues::readDataRegisterSuccess)
												{
													...
													rs->signedIntValue =
														...
														(batteryPower > 0 ? (int32_t)batteryPower : (int32_t)0);
